# Function for initializing the state with 10 random values
# It will return a list
def initialize_state():
    return [2, 1, 5, 0, 8, 4, 10, 0, 20, 10]
...
def calc_cost(_state_: list):
    ...
    for i in range(len(_state_)):
        for j in range(i + 1, len(_state_)):
            # If the value at index i is greater than the value at index j, then increment the cost
...
# Algorithm for hill climbing with steepest ascent
...
if __name__ == '__main__':
    ...
    # Performing hill climbing with steepest ascent, until the goal state is reached
